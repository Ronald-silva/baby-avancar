...
- SEO técnico: `metadata` da Home e de `/acesso` (title/description reais, `metadataBase`, `canonical` por página via `alternates.canonical`), título com template (`%s | Colégio Baby Avançar`).
...
- Google Search Console: verificar propriedade e submeter `sitemap.xml` — só possível com uma URL pública estável (domínio próprio ou, no mínimo, a URL final do Railway).
- Bing Webmaster Tools: mesma dependência de URL pública estável.
...
| 013 | `metadataBase`/canonical/sitemap usam a URL do Railway já validada (ADR-012) como fallback via `NEXT_PUBLIC_SITE_URL`, não um domínio inventado | Não existe domínio próprio ainda (ADR-004 em aberto); usar uma URL real e já confirmada em vez de um placeholder evita metadata/canonical apontando para algo que nunca existiu | Hardcode de um domínio ainda não registrado (rejeitada — violaria "não inventar informação") | 2026-09-12 | Aceita — trocar só a env var quando houver domínio próprio, nenhum código muda |
