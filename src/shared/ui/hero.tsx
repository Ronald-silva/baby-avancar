...
//
// Sem animação de entrada (fade/scale/opacity inicial) na foto nem no bloco
// de texto: os dois são o conteúdo crítico da primeira dobra — a imagem é o
// próprio elemento de LCP. Um `initial={{opacity:0}}` nascia no HTML
// renderizado no servidor (`style="opacity:0"`) e só virava opacity:1 depois
// de hidratar + rodar a transição, empurrando o tempo de LCP para depois
// disso (medido: ~2.6s de "element render delay" no Lighthouse, maior fatia
// do LCP total). Motion continua livre em qualquer seção abaixo da dobra
// (`MotionReveal`, usado por todas as seções depois do Hero).
export function Hero() {
  return (
    <section aria-label="Apresentação do Colégio Baby Avançar" className="relative overflow-hidden bg-ink lg:min-h-[44rem]">
      <div className="relative aspect-[4/3] w-full overflow-hidden sm:aspect-[16/10] lg:absolute lg:inset-y-0 lg:right-0 lg:aspect-auto lg:h-full lg:w-[58%]">
        {/*
      ...
      <div className="px-5 py-10 sm:px-8 sm:py-12 lg:relative lg:mx-auto lg:flex lg:min-h-[44rem] lg:max-w-6xl lg:items-center lg:py-16">
        <HeroContent className="lg:max-w-[46%]" />
      </div>
    </section>
  );
}
