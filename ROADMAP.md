...
1. **Risco de deploy:** `vercel.json` vazio + `package.json` com scripts Next.js pode fazer a Vercel trocar o preset de "site estático" para "Next.js app" e derrubar o site legado em produção (arquivos fora de `public/` deixam de ser servidos). Isso precisa ser resolvido **antes** de qualquer outro trabalho, não depois.
2. **Ausência total de controle de versão na raiz do projeto.** Não há `.git` hoje — qualquer erro de edição é irreversível. Nenhuma fase de código deve começar antes disso ser corrigido.
...
|---|---|---|---|
| 0 — Fundação | Eliminar os dois riscos críticos e provisionar a base de infraestrutura | P | Git iniciado; deploy da Vercel confirmado e seguro; Railway + Postgres provisionados; projeto fantasma removido |
...
### 3.1 Separação frontend/backend

```
Next.js (Vercel) → API (contrato HTTP) → Backend (Railway) → PostgreSQL (Railway)
```

O frontend Next.js **não** acessa o Postgres diretamente e **não** contém regra crítica de negócio (validações de UI são permitidas; decisões de domínio, não). Toda regra de negócio vive no backend Railway, exposta por uma API versionada.
...
- **Ambientes:** no mínimo `staging`/`preview` e `production` separados, tanto no Railway quanto na Vercel, antes de qualquer dado real de terceiros (lead, matrícula) ser gravado.
...
| Configuração de deploy da Vercel confirmada/travada | Decidir explicitamente: site legado descontinuado nesta fase (ver Fase 1) ou mantido temporariamente por trás de um preset "static" enquanto a Fase 1 não termina — **não pode ficar implícito/no acaso** | P |
| Backend Railway provisionado | Serviço Node (ou stack equivalente) + PostgreSQL, ambiente `staging` e `production` separados desde o início (§3.7) | M |
...
| Health check + logs estruturados no backend | Primeiro endpoint do backend já nasce com isso, não é retrofit depois | P |

**Critério de pronto:** `git log` mostra histórico real; Railway com Postgres acessível a partir de um health check simples; Vercel não corre mais risco de trocar de preset sem aviso.

---

## 5. Fase 1 — Institucional & SEO

**Objetivo:** unificar os dois sites em um único app Next.js, sem perder o SEO já conquistado pelo site legado.
...
| Migração de conteúdo do site legado para `src/app` | Todas as seções de `index.html` (Sobre, Ensino, Diferenciais, Depoimentos, Contato) e as páginas `galeria.html`, `cursos-extras.html`, `servicos-assessoria.html` recriadas como rotas Next.js | G |
...
**Critério de pronto:** todas as páginas do site legado têm equivalente no Next.js; Google Search Console (ou verificação manual de JSON-LD/sitemap) confirma paridade de SEO; domínio próprio resolvendo em produção.
...
| Ambientes e backup já operando (§3.7) | Staging/production e backup do Postgres precisam estar de pé **antes** de dados reais de famílias começarem a ser gravados aqui | — |
...
| Momento exato do descomissionamento do site legado | Fase 1 | Depende da confirmação de paridade de conteúdo e SEO, não de uma data fixa |
...
| 004 | Domínio próprio substituirá `baby-avancar.vercel.app` | Mais profissional para uma escola real; melhora SEO local de longo prazo | Manter subdomínio Vercel indefinidamente | 2026-09-11 | Aceita |
| 005 | Projeto fantasma `escola-baby-avancar/` será removido na Fase 0 | Boilerplate não utilizado, sem risco de remoção, elimina confusão de dois `package.json`/`node_modules`/git aninhados | Manter por enquanto | 2026-09-11 | Aceita |
